...
public class TickerJdbcRepository {
    ...
    public void batchUpsert(List<Ticker> tickers) {
        ...
        String sql = "INSERT INTO tickers (" +
                "id, market_id, last_price, price_change, price_change_pct, " +
        ...
        jdbcTemplate.batchUpdate(sql, tickers, tickers.size(), (PreparedStatement ps, Ticker ticker) -> {
            ps.setLong(1, ticker.getId());
            ps.setLong(2, ticker.getMarketId());
            ps.setLong(3, ticker.getLastPrice());
            ps.setLong(4, ticker.getPriceChange());
            ps.setLong(5, ticker.getPriceChangePct());
            ps.setLong(6, ticker.getHigh24h());
            ps.setLong(7, ticker.getLow24h());
            ps.setLong(8, ticker.getVolume24h());
            ps.setLong(9, ticker.getQuoteVolume24h());
            ps.setLong(10, ticker.getTradeCount24h());
        });
    }
}